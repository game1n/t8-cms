...
const Home = (): ReactElement => {
  ...
  const [userDetails, setUserDetails] = useState<UserDetailsResponseType[]>([userDetailsInitialState]);
  ...
  const fetchUserDetails = (): void => {
    setLoading(true);
    getUserDetails(session?.user.id as string).then((response) => {
  ...
  const updateUserDetails = (requestPayload: UserDetailsPayloadType): void => {
    postUserDetails(requestPayload).then((response) => alert('data updated successfully')).catch((error) => console.error(error));
